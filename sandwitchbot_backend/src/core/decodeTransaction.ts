...
const uniswapUniversalTransaction = async (transaction: Transaction, activedTokenLists: TokenProps[]): Promise<BaseTransactionParamerterProps | undefined> => {
  let decoded;
  try {
  ...
  const targetToken = decoded.path[1]; //checkTransactionForTarget(decoded.path[1], activedTokenLists);
...
const uniswapV2Transaction = async (transaction: Transaction, activedTokenLists: TokenProps[]): Promise<BaseTransactionParamerterProps | undefined> => {
  let decoded;
  try {
...
const decodeTransaction = async (transaction: Transaction, activedTokenLists: TokenProps[]): Promise<any | undefined> => {
